...
**設定は既存の `fanaby-event-register` アプリケーションに `assets/*` エントリを追加する形で行うこと（新規アプリケーションを作らない）。** 2026-08-31 に検証した際、`assets/*` を宛先に持つ新規アプリケーションを別途作成しても bypass が機能しなかった（30分待っても解消せず、伝播遅延ではなかった）。一方、同じ `assets/*` エントリを既存の `fanaby-event-register`（`/register.html` bypass 用。以前から正常に機能している）に追加したところ即座に機能した。原因は特定できていないが、Cloudflare Access の複数アプリケーション優先順位は新規アプリケーションでは期待通りに働かないことがあるため、bypass 系のパスは実績のある既存アプリケーションに追加していく方針とする。
